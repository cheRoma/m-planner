...
- ✅ **План 2 реализован и смёржен в main** (subagent-driven, merge `aa16037`, 2026-05-30). Acquisition-спина: `POST /projects` save-as-budget, идемпотентный `SpendFactService`, crowd-агрегация (винзоризация+перцентили, BullMQ), публичная `GET /public/showcase` (k-anon `BenchmarkReader` + Redis-кэш + Cache-Control), `RateLimitGuard`, `MetricsService`, `apps/miniapp` (Next.js: UX сметы + SSR `/showcase` + OG `/s/[preset]`).
- ✅ **План 3 реализован и смёржен в main** (subagent-driven, merge `80ba1ce`, 2026-05-30). Планировщик: бюджет-трекер + платежи с **optimistic-lock** (409), платежи дёргают `SpendFactService.recompute` (enqueue на upsert+delete), делегирование **secure invite-токеном** (атомарный one-time claim в транзакции + authz владельца), ЗАГС/таймлайн-чеклист (compliance: source/review/city) инстанцируется при создании проекта, камерный режим (format-profile). **Authz:** все budget/checklist пути проверяют членство в проекте (`assertMember`). Тесты: `@m/api` **60**, `@m/shared` 7, `@m/db` 4 — зелёные (turbo 8/8). Прошло по-блочные + финальное ревью (финал поймал и закрыл authz-дыру на budget/checklist).
- ⏳ **План 4 не начат** (напоминания: BullMQ-доставка через grammY-бот, retries/429/blocked, fallback-view, 152-ФЗ consent+delete, seed-refresh, 3 крит. integration-теста).

**Известные follow-up (в `docs/TODOS.md`):** (1) prod `build`/`start` у `@m/api` (ESM-source-workspace); (2) платёж+recompute не транзакционны → outbox/after-commit enqueue (План 4); (3) share-URL port-rewrite в miniapp → `NEXT_PUBLIC_WEB_URL` до прода; (4) ЗАГС-шаблоны требуют human verify-before-launch (named owner, §12.12); (5) reminder send-hour (03:00 МСК артефакт) + skip past-reminders — План 4.
