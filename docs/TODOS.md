...
- [x] **Server-side consent enforcement (152-ФЗ)** — ✅ СДЕЛАНО и доказано рантаймом (merge `8442153`; fix-chain `4087f18` AUTH_CONFIG-провайдер → `fd471e2` импорты контроллера+@UseGuards → `d228d33` ProjectModule импортит ConsentModule). `ConsentGuard` (`apps/api/src/consent/consent.guard.ts`) на `ProjectController` через `@UseGuards(JwtGuard, ConsentGuard)`: нет consent → 403. **Доказано живым сервером:** no-consent→403, POST /consent→201, after-consent→201, no-token→401. turbo 10/10 (api 80), build RC=0. _Урок (трижды!): «зелёные unit-тесты, сломанный рантайм» — выпавший AUTH_CONFIG-провайдер; слипшиеся импорты уронили @UseGuards; ProjectModule не импортил ConsentModule (Nest резолвит guard в контексте модуля контроллера). Все три поймал ТОЛЬКО живым curl + e2e-бутстрапом, не unit-тестами. ConsentGuard unit-тест был зелёным всё это время. Всегда runtime-smoke для guard/DI-изменений._
