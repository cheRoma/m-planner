...
- [x] **Server-side consent enforcement (152-ФЗ)** — ✅ СДЕЛАНО (merge `8442153`, fixes `4087f18` AUTH_CONFIG + `b9f7e3c` controller wiring). `ConsentGuard` (`apps/api/src/consent/consent.guard.ts`) на `ProjectController` через `@UseGuards(JwtGuard, ConsentGuard)`: нет consent → 403. **Доказано рантаймом** (живой сервер: no-consent→403, POST /consent→201, after-consent→201, no-token→401). Покрывает создание проекта = точку первого хранения перс.данных (budget/checklist/invite требуют существующего проекта). Тесты turbo 10/10, build RC=0. _Урок: два раза словил «зелёные тесты, сломанный рантайм» (выпавший AUTH_CONFIG-провайдер при перезаписи модуля; слипшиеся импорты в контроллере уронили @UseGuards) — оба поймал только живым curl, не тестами._
